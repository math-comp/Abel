...
have ilt k (i : 'I_k.-1) : (i < k)%N by rewrite ltnW ?Silt.
...
have peq (i : 'I_n.-1) : p.[s`_i] = p.[s`_i.+1] by rewrite !peq0.
...
apply/(sortedP 0); rewrite size_tuple => i' i'_lt2; pose i := Ordinal i'_lt2.
pose i1 := Ordinal (ilt _ i); pose Si1 := Ordinal (Silt _ i).
by rewrite (rtE i1) (rtE Si1) (@lt_trans _ _ s`_Si1)// (itvP (rs _)).
Qed.
